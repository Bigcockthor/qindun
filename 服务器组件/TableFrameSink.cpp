...
bool __cdecl CTableFrameSink::OnEventGameStart()
{
...
#ifdef _DEBUG
	m_wCurrentUser=0;
	BYTE card1[41]={0x02,0x06,0x07,0x08,0x09,0x0a,0x06,0x16,0x26,0x36,0x04,0x04,0x05,0x25,0x15,0x25,0x26,0x26,0x16,0x16,0x16,
					0x08,0x09,0x09,0x09,0x0a,0x0a,0x1a,0x1b,0x1b,0x2b,0x2b,0x2c,0x1c,0x1d,0x1d,0x11,0x11,0x11,0x11,0x11};
	BYTE card2[41]={0x03,0x03,0x13,0x13,0x33,0x23,0x04,0x35,0x25,0x15,0x05,0x05,0x06,0x26,0x16,0x26,0x27,0x27,0x17,0x18,0x18,
					0x08,0x08,0x18,0x09,0x0a,0x0a,0x1a,0x1b,0x1b,0x2b,0x2b,0x2c,0x1c,0x1d,0x12,0x12,0x12,0x11,0x11,0x11};
	BYTE card3[40]={0x02,0x02,0x02,0x12,0x12,0x22,0x04,0x04,0x05,0x05,0x05,0x05,0x06,0x26,0x16,0x26,0x27,0x27,0x17,0x18,
					0x08,0x08,0x08,0x09,0x09,0x09,0x1a,0x1a,0x1a,0x2b,0x2b,0x2c,0x1c,0x1d,0x12,0x12,0x12,0x11,0x11,0x11};
	BYTE card4[40]={0x11,0x11,0x11,0x12,0x12,0x12,0x0d,0x0d,0x05,0x05,0x05,0x05,0x06,0x26,0x16,0x26,0x27,0x27,0x17,0x17,
					0x08,0x08,0x18,0x18,0x28,0x28,0x38,0x38,0x1a,0x2b,0x2b,0x2c,0x1c,0x1d,0x12,0x12,0x12,0x13,0x13,0x13};

	CopyMemory(m_cbHandCardData[0],card1,sizeof(BYTE)*41);
	CopyMemory(m_cbHandCardData[1],card2,sizeof(BYTE)*41);
	CopyMemory(m_cbHandCardData[2],card3,sizeof(BYTE)*40);
	CopyMemory(m_cbHandCardData[3],card4,sizeof(BYTE)*40);
#endif
	////////////////////////////////////////////////////////////////////
	//设置逻辑
	...
	//LONG lScoreAll=0;
	//lScoreAll=m_GameLogic.GetCardScore(m_cbHandCardData[0],m_cbHandCardCount[0]) 
	//	+m_GameLogic.GetCardScore(m_cbHandCardData[1],m_cbHandCardCount[1])
	//	+m_GameLogic.GetCardScore(m_cbHandCardData[2],m_cbHandCardCount[2])
	//	+m_GameLogic.GetCardScore(m_cbHandCardData[3],m_cbHandCardCount[3]);

	//ASSERT(lScoreAll==300);
	...
	case GER_NORMAL:		//常规结束
		{
			...
			else
			{
				...
				//贡献信息
				TCHAR szMessage[512]=TEXT("");
				_sntprintf(szMessage,CountArray(szMessage),TEXT("玩家[ %s ]的得分 %d 和手中牌分 %d 贡献给对方。扣除其对家 [ %s ]30分\n"),
							m_pITableFrame->GetServerUserItem(m_wWinOrder[3])->GetAccounts(),m_PlayerScore[m_wWinOrder[3]],LeaveScore,
							m_pITableFrame->GetServerUserItem((m_wWinOrder[3]+2)%GAME_PLAYER)->GetAccounts());
				//发送贡献信息
				for (WORD i=0;i<GAME_PLAYER;i++)
				{
					pIServerUserItem=m_pITableFrame->GetServerUserItem(i);
				...
				//下游分数给对方
				m_PlayerScore[(m_wWinOrder[3]+1)%GAME_PLAYER]+=m_PlayerScore[m_wWinOrder[3]]+LeaveScore;
				m_PlayerScore[m_wWinOrder[3]]=0;
				//输家的对家扣除30分
				m_PlayerScore[(m_wWinOrder[3]+2)%GAME_PLAYER]-=30;
			}
			...
			if (bStatScore==true)
			{
				...
				//负分情况
				if( (m_PlayerScore[1]+m_PlayerScore[3])<0 )
				{
				...
				else if( (m_PlayerScore[0]+m_PlayerScore[2])<0 )
				{
					lUserScore[0]=-2L*lCellScore;
					lUserScore[2]=-2L*lCellScore;
					lUserScore[1]=2L*lCellScore;
				...
				if( (m_wWinOrder[0]==0)&&(m_wWinOrder[1]==2) || (m_wWinOrder[0]==2)&&(m_wWinOrder[1]==0) )
				{
					lUserScore[0]=3L*lCellScore;
					lUserScore[2]=3L*lCellScore;
					lUserScore[1]=-3L*lCellScore;
					lUserScore[3]=-3L*lCellScore;
				}
				else if( (m_wWinOrder[0]==1)&&(m_wWinOrder[1]==3) || (m_wWinOrder[0]==3)&&(m_wWinOrder[1]==1) )
				{
					lUserScore[0]=-3L*lCellScore;
...
bool CTableFrameSink::OnUserOutCard(WORD wChairID, BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if(m_cbTurnCardCount==0)
	{
		//检测是否全是2
		bool bPure2=true;
		for(BYTE i=0;i<cbCardCount;i++) if(m_GameLogic.GetCardValue(cbCardData[i])!=2) bPure2=false;
	...
	if (m_cbTurnCardCount!=0)
	{
		...
		//对比扑克
		else if (m_GameLogic.CompareCard(m_cbMagicCardData,cbMagicCardData,m_cbTurnCardCount,cbCardCount)==false)
		{
			ASSERT(FALSE);
			return false;
		}
